 ...
 *
 */
public class RemoveStaffQualification implements BotCommand{

	@Override
	public String getHelp() {
		return "Removes a qualfication from the given user. Use the format: "+this.getShortHelp();
	}

	@Override
	public String getShortHelp() {
		return "<command> <Staff name> <qualification to remove>";
	}
	...
	public void execute(MessageEvent event, String argString) 
	{
		...
		if(isVerified && user.hasPrivilege(Privilege.ADD_QUALIFICATION))	//if they're verified AND it's the registered user OR someone with ADD_QUALIFICATION privilege.
		{
			...
			if(userDatabase.removeQualificationFromStaffMember(args[0], AppDictionary.getSharedInstance().getSubstitutionAgainstContext(args[1], "Qualifications")))
			{
				event.respondChannel(StaffMember.deHighlightUsername(args[0])+" no longer has the qualification: "+args[1] );
			}
			else
			{
